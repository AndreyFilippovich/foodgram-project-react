...
class FollowRecipeSerializers(serializers.ModelSerializer):
    """
    Сериализатор для отображения списка рецептов в FollowUserSerializers.
    """
    class Meta:
        model = Recipe
...
class FollowUserSerializers(serializers.ModelSerializer):
    """
    Сериализатор подписчиков с дополнительными полями:
    is_subscribed - подписка на автора (True);
    recipes - рецепты автора;
    recipes_count - количество рецептов автора;
    Реализована пагинация рецептов.
    """
...
class TagSerializers(serializers.ModelSerializer):
    """
    Сериализатор тегов для рецептов.
    """
    class Meta:
        model = Tag
        fields = '__all__'
...
class IngredientRecipeSerializers(serializers.ModelSerializer):
    """
    Сериализатор для ингридиентов для рецптов.
    """
...
class RecipeSerializers(serializers.ModelSerializer):
    """
    Сериализатор рецептов с дополнительными полями:
    1- Поля избранного (False/True);
    2- Поля списка покупок (False/True);
    Проверка корректности добавляемых ингридиентов,
    Способ добавления ингриниентов в рецепт:
    1- Создание рецепта;
    2- Изменение рецепта;
    """
...
class FavoriteSerializers(serializers.ModelSerializer):
    """
    Сериализатор выбранных рцептов.
    """
